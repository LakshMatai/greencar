...
public class AssessmentModelDefinition : NeuralModelDefinition {
    ...
    public typealias Output = (String, [String : Double])
    ...
    public var path: String = CarRecognition.urlOfModelInThisBundle.path
    ...
    public func postprocess(outputs: [String : Any], input: MTLTexture, parameters: [String : Any]) -> (String, [String : Double])? {
        //print(outputs)
        //Access the outputs relative to the model you are using
        guard let label = outputs["classLabel"] as? String else{
            return nil
        }
        guard let probs = outputs["prob"] as? [String : Double] else{
            return nil
        }
